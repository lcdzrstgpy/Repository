...
- **单仓模式**：面向小公司，仅使用一个内部默认仓；页面和日常操作不展示仓库选择。
- **运营**：录入订单、确认每个 SKU 的最终数量；数量确认阶段不校验库存。
- **仓储**：接单时关联/新建货号，完成采购、入库与发货；库存不足由仓储采购解决。
- **管理员**：只做全局监管和主数据维护，菜单仅有「全部订单、采购单、货号管理、人员与权限」；订单与采购单均为只读，不能代替运营或仓储执行流程。

### 已知限制

1. **SQLite 下并发防护失效** —— 开发模式用 SQLite 时 `SELECT ... FOR UPDATE` 会被静默忽略，行锁不生效。生产必须使用 MySQL。
2. **`prepare` 接口已废弃** —— 保留路由避免旧客户端 404，但调用返回 `code=1001`；新流程以运营的 `confirm-quantity` 进入备货。
3. **`approver` 为历史兼容角色** —— 当前采购不要求审批，日常采购由仓储完成并入库。
...
3. 按 **「四、快速开始」** 把服务跑起来
4. 跑 **`backend/tests/smoke_test.py`**（86 项断言，不需要 MySQL）确认环境没问题
...
```
                    ...
                    │                        ▲
                    │                        │
 待接单(10) ──接单──▶ 已接单(20) ──绑完货号──▶ 数量待确认(25) ──运营确认──▶ 备货中(30) ──发货──▶ 已完成(50)
                      （系统自动）              （系统自动，同时预留库存）
```
...
| 20 | 已接单 | 仓储接单 | 写 `claimed_by` / `claimed_at`；**此阶段处理货号关联** |
| 25 | 数量待确认 | 系统自动（仓储绑完所有货号） | 等运营确认数量 |
| 30 | 备货中 | 系统自动（运营确认数量） | 写 `prepare_at`；仓储备货或采购，尚不扣减/预留库存 |
| 50 | 已完成 | 仓储发货 | 仓储发货即完成，写 `shipped_at` / `express_no` / `finished_at` |
...
# 固定使用项目 Conda 环境（Python 3.12.14）
conda activate cangchu
python -m pip install -r requirements.txt
...
| 用户名 | 姓名 | 角色 | 权限说明 |
|---|---|---|---|
| `admin` | 管理员 | `admin` | 全部订单与采购单只读查看；维护货号主数据、商品名称、人员与权限 |
| `operator1` | 运营小王 | `operator` | 创建订单、**只能看自己创建的订单**、取消自己的订单、确认数量 |
| `warehouse1` | 仓管老李 | `warehouse` | 接单、关联/新建货号、库存查询与调整、采购完成入库、发货 |

> `approver` 是历史兼容角色，当前采购流程不要求审批；仓储人员可直接完成采购并入库。
...
| 表名 | 说明 |
|---|---|
| `purchase_order` | 采购单（`status`：10 待采购 / 30 已入库 / 90 已取消；20 为历史兼容状态） |
...
```
库存不足 → 创建采购单(10 待采购) → 仓储采购完成并入库(30 已入库) → 库存增加 → 订单继续备货
```
...
- `GET /api/inventory/alerts` 返回可用量低于安全库存下限的库存记录，按短缺量从大到小排序。
...
- [`backend/tests/smoke_test.py`](backend/tests/smoke_test.py) —— 端到端冒烟测试（86 项断言，**不需要 MySQL**，改完代码可随时回归）
