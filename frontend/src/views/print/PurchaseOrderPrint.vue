  ...
  <div class="print-page">
    ...
    <div v-loading="loading" class="paper">
      <template v-if="detail">
        <h1 class="doc-title">采 购 单</h1>

        <div class="doc-meta">
          <div class="meta-item">
            <span class="meta-label">采购单号：</span>
          ...
          <div class="meta-item">
            ...
            <span class="meta-value">{{ detail.sales_order_no || '-' }}</span>
          </div>
          <div class="meta-item">
          ...
          <div class="meta-item">
            ...
            <span class="meta-value">{{ detail.approved_by_name || '-' }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">单据状态：</span>
            <span class="meta-value">{{ detail.status_text || purchaseStatusLabel(detail.status) }}</span>
          </div>
        </div>
        ...
        <table class="detail-table">
          ...
          <tbody>
            ...
            <tr v-if="!detail.items || !detail.items.length">
              <td class="cell-center empty-cell" colspan="7">暂无采购明细</td>
            </tr>
            <!-- 合计行 -->
      ...
      <el-empty
        v-else-if="!loading"
        description="未获取到该采购单数据，请确认单据是否存在或已被删除"
      />
    </div>
  </div>
</template>
...
import { purchaseStatusLabel } from '@/utils/constants'
...
/** 拉取采购单详情（复用已有详情接口，契约 13） */
async function load() {
  loading.value = true
  try {
    detail.value = await getPurchaseDetail(route.params.id)
  } catch (e) {
