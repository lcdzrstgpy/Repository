...
Five event kinds, each with its own metric:
  ...
  picking       COUNT(DISTINCT entity_id) WHERE entity_type='SO'
...
DASHBOARD_EVENTS: List[Tuple[str, str, str]] = [
    ...
    # picking measures distinct SOs picked, not summed line
...
def _agg_picking(db, warehouse_id, start, end):
    # Count distinct SOs the user closed, not summed units. PICK rows
    # are written one-per-line at pick-confirm time with
