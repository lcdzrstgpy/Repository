...
Modified for Hub ERP integration, 2026-09-14: inbound order lines resolve
only active item codes.
...
def _prefetch_line_lookups(db, document, resource_key, source_payload):
    ...
    for (ss, st), sids in groups.items():
        found: Dict[str, UUID] = {}
        if st == "item":
            # Hub draft codes exist for review but must never enter a sales
            # order. Existing items default to active in migration 082.
            lookup_sql = (
                "SELECT csm.source_id, csm.canonical_id "
                "  FROM cross_system_mappings csm "
                "  JOIN items i ON i.external_id = csm.canonical_id "
                " WHERE csm.source_system = :ss AND csm.source_type = :st "
                "   AND csm.source_id = ANY(:sids) "
                "   AND i.code_status = 'active' AND i.is_active = TRUE"
            )
        else:
            lookup_sql = (
...
def handle_inbound(
    *,
    ...
    def _lookup(ss: str, st: str, sid: str) -> Optional[UUID]:
        ...
        if st == "item":
            lookup_sql = (
                "SELECT csm.canonical_id FROM cross_system_mappings csm "
                " JOIN items i ON i.external_id = csm.canonical_id "
                " WHERE csm.source_system = :ss AND csm.source_type = :st "
