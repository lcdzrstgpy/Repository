...
class TestProductivityAggregation:
    def test_picking_distinct_orders(self, client, auth_headers):
        # Picking metric is "distinct SOs picked", so two PICK lines on
        # the same SO collapse to one credit. alice picks two different
        # SOs (one of them across two lines); bob picks one.
...
class TestCache:
    def test_repeat_call_within_ttl_does_not_re_query(self, client, auth_headers):
        ...
        # Picking is distinct-SO so we seed on a specific so_id.
        _seed_audit("PICK", "alice", 1,
                    {"quantity_picked": 5, "sku": "A", "item_id": 1},
