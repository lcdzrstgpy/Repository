"""Schema-level tests for migration 042 (v1.7.0 inbound_vendors + new canonical vendors).

Same coverage shape as test_inbound_customers_migration.py:
- vendors exists with canonical_id UUID PK + V-216 external_id UUID UNIQUE
...
- inbound_vendors shape mirrors mig 039 / 040 / 041
"""
...
def _make_token(conn):
    ...
    cur.execute(
        ...
        "RETURNING token_id",
        (
            f"inbound-vendors-test-{uuid.uuid4().hex[:8]}",
...
class TestCanonicalVendorsShape:
    def test_table_exists_with_conservative_not_null_posture(self):
        ...
        for col in ("vendor_name", "contact_name", "email", "phone",
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT column_default FROM information_schema.columns
                 WHERE table_name = 'vendors' AND column_name = 'canonical_id'
                """
            )
    ...
    def test_external_id_is_unique_with_uuid_default(self):
        ...
        try:
            ...
            cur.execute(
                """
                SELECT column_default FROM information_schema.columns
                 WHERE table_name = 'vendors' AND column_name = 'external_id'
                """
            )
            default = cur.fetchone()[0]
            cur.execute(
    ...
    def test_minimal_insert_satisfies_conservative_not_null_set(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO vendors DEFAULT VALUES RETURNING canonical_id, external_id, "
                "       vendor_name, email, phone, latest_inbound_id"
            )
            row = cur.fetchone()
            cid = row[0]
            cur.execute("DELETE FROM vendors WHERE canonical_id = %s", (cid,))
        finally:
            conn.close()
    ...
    def test_table_and_indexes_present(self):
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT column_name, data_type, is_nullable
                  FROM information_schema.columns
                 WHERE table_name = 'inbound_vendors'
                """
            )
            cols = {r[0]: (r[1], r[2]) for r in cur.fetchall()}
            cur.execute(
                "SELECT indexname FROM pg_indexes WHERE tablename = 'inbound_vendors'"
            )
        ...
        assert "inbound_vendors_idempotency" in indexes
        assert "inbound_vendors_current" in indexes
        assert "inbound_vendors_canonical" in indexes

    def test_idempotency_unique_rejects_duplicate(self):
        ss = f"vendors-uniq-{uuid.uuid4().hex[:8]}"
        conn = _make_conn()
        conn.autocommit = True
        try:
            ...
            try:
                ...
                cur.execute("DELETE FROM inbound_vendors WHERE source_system = %s", (ss,))
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT rc.delete_rule, ccu.table_name
                  ...
                  JOIN information_schema.constraint_column_usage ccu
                    ON tc.constraint_name = ccu.constraint_name
                 WHERE tc.table_name = 'inbound_vendors'
