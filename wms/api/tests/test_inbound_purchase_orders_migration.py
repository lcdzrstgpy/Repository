"""Schema-level tests for migration 043 (v1.7.0 inbound_purchase_orders).

Same shape as test_inbound_sales_orders_migration.py and
test_inbound_items_migration.py.
"""
...
def _make_token(conn):
    ...
    cur.execute(
        ...
        "RETURNING token_id",
        (
            f"inbound-po-test-{uuid.uuid4().hex[:8]}",
...
class TestInboundPurchaseOrdersShape:
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT column_name, data_type, is_nullable
                 ...
                 WHERE table_name = 'inbound_purchase_orders'
    ...
    def test_indexes_present(self):
        ...
        try:
            ...
            cur.execute(
                ...
                " WHERE tablename = 'inbound_purchase_orders'"
            )
            indexes = {name: defn for name, defn in cur.fetchall()}
        finally:
            conn.close()
        assert "inbound_purchase_orders_idempotency" in indexes
        idem = indexes["inbound_purchase_orders_idempotency"]
        assert "UNIQUE" in idem.upper()
        for col in ("source_system", "external_id", "external_version"):
            assert col in idem
        assert "inbound_purchase_orders_current" in indexes
        cur_def = indexes["inbound_purchase_orders_current"]
        assert "received_at" in cur_def
        assert "WHERE" in cur_def.upper() and "applied" in cur_def
        assert "inbound_purchase_orders_canonical" in indexes
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT rc.delete_rule, ccu.table_name
                 ...
                 WHERE tc.table_name = 'inbound_purchase_orders'
...
class TestInboundPurchaseOrdersBehavior:
    def _insert_row(self, conn, ss, token_id, ext_id, ext_ver):
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO inbound_purchase_orders "
            ...
            (
                ...
                ext_ver,
                str(uuid.uuid4()),
                Json({"po_number": ext_id}),
                Json({"poNumber": ext_id}),
                token_id,
            ),
        )
    ...
    def test_idempotency_unique_rejects_duplicate(self):
        ss = f"po-uniq-{uuid.uuid4().hex[:8]}"
        conn = _make_conn()
        conn.autocommit = True
        try:
            _ensure_allowlist(conn, ss)
            token_id = _make_token(conn)
            self._insert_row(conn, ss, token_id, "PO-1", "v1")
            try:
                self._insert_row(conn, ss, token_id, "PO-1", "v1")
            except psycopg2.errors.UniqueViolation:
                cur = conn.cursor()
                cur.execute("DELETE FROM inbound_purchase_orders WHERE source_system = %s", (ss,))
                cur.close()
    ...
    def test_status_check_rejects_bogus(self):
        ss = f"po-status-{uuid.uuid4().hex[:8]}"
        conn = _make_conn()
        conn.autocommit = True
        try:
            _ensure_allowlist(conn, ss)
            ...
            try:
                cur.execute(
                    ...
                    "VALUES (%s, 'PO-x', 'v1', %s, '{}'::jsonb, '{}'::jsonb, %s, 'pending')",
    ...
    def test_token_delete_with_inbound_row_raises_restrict(self):
        ss = f"po-tokfk-{uuid.uuid4().hex[:8]}"
        conn = _make_conn()
        conn.autocommit = True
        try:
            _ensure_allowlist(conn, ss)
            token_id = _make_token(conn)
            inbound_id = self._insert_row(conn, ss, token_id, "PO-tokfk", "v1")
            cur = conn.cursor()
            try:
                cur.execute("DELETE FROM wms_tokens WHERE token_id = %s", (token_id,))
            except psycopg2.errors.ForeignKeyViolation:
                cur.execute("DELETE FROM inbound_purchase_orders WHERE inbound_id = %s", (inbound_id,))
                _drop_token(conn, token_id)
                _drop_allowlist(conn, ss)
                return
...
class TestPurchaseOrdersLatestInboundId:
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT COUNT(*) FROM information_schema.constraint_column_usage ccu
                 ...
                 WHERE ccu.table_name = 'purchase_orders'
            ...
            conn.close()
        assert row is not None, "purchase_orders.latest_inbound_id missing"
